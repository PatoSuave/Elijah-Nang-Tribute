...
export default async function MemorialAdminPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  if (!validAdminSession(session)) return <section className="admin-page"><p className="section-label">Private administration</p><h1>Messages for Elijah</h1><AdminLogin /></section>;
  ...
  return <section className="admin-page"><p className="section-label">Private administration</p><h1>Messages for Elijah</h1><p>Review each submission with care before publishing it to the archive.</p>{error ? <p role="alert">{error}</p> : <><AdminQueue entries={result!.messages} csrf={csrfToken(session!)} emptyMessage={page > 1 ? "No messages are awaiting review on this page." : undefined} /><nav className="admin-pagination" aria-label="Moderation queue pages">{page > 1 && <a className="button-secondary" href={`/admin/memorial?page=${page - 1}`}>Newer submissions</a>}{result!.hasMore && page < maximumPage && <a className="button-secondary" href={`/admin/memorial?page=${page + 1}`}>Older submissions</a>}</nav></>}</section>;
}
