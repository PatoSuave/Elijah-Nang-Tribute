...
export async function POST(request: Request) {
  ...
  if (typeof body.website === "string" && body.website.trim()) return NextResponse.json({ error: "We could not receive this message." }, { status: 400 });
  const parsed = validateMemorialSubmission(body);
  ...
  if (!(await verifyTurnstile(body.turnstileToken, remoteIp ?? undefined))) return NextResponse.json({ error: "Please complete the verification and try again." }, { status: 400 });
  try {
